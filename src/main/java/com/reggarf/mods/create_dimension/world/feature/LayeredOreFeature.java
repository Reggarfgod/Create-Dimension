...
public class LayeredOreFeature extends Feature<NoneFeatureConfiguration> {
        ...
        for (int dx = -rX; dx <= rX; dx++) {
            ...
            for (int dz = -rZ; dz <= rZ; dz++) {
                ...
                for (int dy = -rY; dy <= rY; dy++) {
                    ...
                    if (distSq <= 1.0D) {
                        if (distSq > 0.55D && random.nextFloat() < 0.40f) {
                            continue;
                        }
        ...
        for (Holder<Block> holder : optionalHolderSet.get()) {
            ...
            switch (layerType) {
                ...
                case "deepslate":
                    ...
                    break;
                case "stone":
                    // If it is not specifically nether, end, or deepslate, it belongs to the main stone layer
    ...
    private OreConfig createConfigForOre(Block block) {
        ...
        if (name.contains("debris") || name.contains("netherite")) {
            ...
            // Modded standard ores (e.g. Tin, Lead, Silver, Osmium, Nickel, Aluminum, Uranium, Fluorite, etc.)
            veinSize = 5;
            veinsPerChunk = 1.2f;
        }
